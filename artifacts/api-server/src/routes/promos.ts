...
import { hashPromoCode, normalizePromoCode } from "../lib/promos";
...
router.get("/admin/promo-codes", admin, async (_req, res) => {
  const { data, error } = await supabaseAdmin
    .from("promo_codes")
    .select("id, code_prefix, discount_type, discount_value, starts_at, ends_at, max_uses, max_uses_per_client, services, active, created_at")
    .order("created_at", { ascending: false });
  if (error) return res.status(500).json({ error: "Impossible de charger les codes promo." });
  return res.json({ promo_codes: data || [] });
});
...
router.post("/admin/promo-codes", admin, async (req: AuthedRequest, res) => {
  ...
  }).select("id, code_prefix, discount_type, discount_value, starts_at, ends_at, max_uses, max_uses_per_client, services, active").single();
  ...
  return res.status(201).json({ code, promo_code: data });
});

router.patch("/admin/promo-codes/:id", admin, async (req: AuthedRequest, res) => {
  ...
  return res.json({ promo_code: data });
});
