...
const verifyPaymentLimiter = rateLimit({
  ...
  message: {
    error: "Trop de vérifications de paiement, réessayez dans une minute.",
  },
});
...
function buildSlickpayItems(
  ...
  for (const item of orderItems) {
    ...
    if (typeof unitPrice !== "number" || !Number.isFinite(qty) || qty <= 0) {
      console.warn(
        `[invoice] Ligne non résolvable pour SlickPay : ${item?.name}`,
      );
      return null;
    }
...
function frontendReturnUrl(orderId: string): string | null {
  const configured =
    process.env.FRONTEND_URL || "https://aura-stream.netlify.app";
  try {
...
function isAllowedPaymentUrl(value: unknown): value is string {
  ...
  try {
    ...
    return (
      url.protocol === "https:" &&
      (url.hostname === "slick-pay.com" ||
        url.hostname.endsWith(".slick-pay.com"))
    );
  } catch {
    return false;
  }
  ...
  async (req: Request, res: ExpressResponse): Promise<void> => {
    try {
      ...
      if (
        ...
        !/^ORD-[A-Za-z0-9-]{6,40}$/.test(order_id)
      ) {
      ...
      const { data: order, error: fetchError } = await supabase
        .from("orders")
        .select(
          "order_id, assigned_email, amount, status, payment_status, slickpay_invoice_id, items",
        )
      ...
      if (fetchError || !order) {
        req.log?.warn(
          { orderId: order_id },
          "Invoice requested for unknown order",
        );
        res
          .status(404)
          .json({
            error:
              "Commande introuvable. Si vous venez de la créer, veuillez réessayer dans quelques secondes.",
          });
        return;
      }
      ...
      if (
        typeof order.slickpay_invoice_id === "string" &&
        order.slickpay_invoice_id.startsWith("pending:")
      ) {
        const claimedAt = Number(order.slickpay_invoice_id.split(":")[1]);
        if (
          Number.isFinite(claimedAt) &&
          Date.now() - claimedAt > 2 * 60 * 1000
        ) {
      ...
      if (order.slickpay_invoice_id) {
        res
          .status(409)
          .json({
            error: "Un paiement a déjà été initialisé pour cette commande.",
          });
        return;
      }
      ...
      const slickpayItems =
        detailed ?? singleLineFallback(order.order_id, amount);

      const itemsTotal = slickpayItems.reduce(
      ...
      const finalItems =
        itemsTotal === amount
          ? slickpayItems
          : singleLineFallback(order.order_id, amount);

      const apiKey =
        process.env.SLICKPAY_PUBLIC_KEY || process.env.SLICKPAY_API_KEY || "";
      const webhookUrl = process.env.SLICKPAY_WEBHOOK_URL || "";
      const configuredAccountValue = String(
        process.env.SLICKPAY_ACCOUNT_UUID || "",
      ).trim();
      ...
      if (
        ...
        res
          .status(503)
          .json({ error: "Le service de paiement n'est pas configuré." });
        return;
      }
      ...
      if (claimError || !claimed?.length) {
        res
          .status(409)
          .json({
            error: "Un paiement est dÃƒÂ©jÃƒÂ  en cours pour cette commande.",
          });
        return;
      }
      ...
      if (!spRes.ok) {
        ...
        req.log?.warn(
          { status: spRes.status, orderId: order_id },
      ...
      if (
        ...
        res
          .status(502)
          .json({ error: "Réponse invalide du prestataire de paiement." });
        return;
      }
      ...
      if (invoiceUpdateError || !saved?.length) {
        req.log?.error(
          { orderId: order_id },
          "Could not persist SlickPay invoice id",
        );
        await notifyAdmin(
          "Facture SlickPay créée mais impossible de l'enregistrer.",
          {
            level: "critical",
        ...
        res
          ...
          .json({
            error:
              "Le paiement ne peut pas ÃƒÂªtre initialisÃƒÂ©. Contactez le support.",
          });
        return;
      }
  ...
  async (req: Request, res: ExpressResponse): Promise<void> => {
    try {
      ...
      const { data: order, error: orderError } = await supabase
        .from("orders")
        .select(
          "order_id, assigned_email, amount, status, payment_status, slickpay_invoice_id, items, expires_at",
        )
        .eq("order_id", orderId)
        .single();
      ...
      if (!order.slickpay_invoice_id) {
        res.status(409).json({
          ...
          error:
            "Cette ancienne facture ne possède pas d'identifiant de vérification.",
        });
        return;
      }
      ...
      if (!apiKey) {
        res
          .status(500)
          .json({ error: "Configuration de paiement incomplète." });
        return;
      }

      const spRes = await fetch(
        `${SLICKPAY_URL}/${encodeURIComponent(order.slickpay_invoice_id)}`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
      ...
      if (!spRes.ok) {
        ...
        req.log?.error(
          { slickpayStatus: spRes.status, detail, orderId },
      ...
      if (
        ...
        providerAmount !== Number(order.amount)
      ) {
        req.log?.error(
          { orderId, expected: order.amount, received: providerAmount },
          "SlickPay amount mismatch",
        );
        await notifyAdmin(
          `Montant SlickPay incohérent pour la commande ${orderId}.`,
          {
            level: "critical",
            orderId,
        ...
        res
          .status(409)
          .json({
            error: "Le montant vérifié ne correspond pas à la commande.",
          });
        return;
      }
      ...
      if (paidUpdateError) {
        req.log?.error(
          { paidUpdateError, orderId },
          "Could not persist verified payment",
        );
        res
          .status(500)
          .json({
            error:
              "Paiement confirmé mais enregistrement impossible. Le support a été alerté.",
          });
        return;
      }
      ...
      if (assignmentError) {
        const waitingForStock =
          assignmentError.message?.includes("OUT_OF_STOCK");
        await notifyAdmin(
          waitingForStock
        ...
        res.status(502).json({
          ...
          order_status: "pending",
          error:
            "Paiement confirmé. L'attribution sera finalisée par le support.",
        });
        return;
      }
